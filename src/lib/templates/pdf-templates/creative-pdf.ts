...
  }
  ...
  if (skillSections.length > 0 && sidebarY < pageHeight - 50) {
    ...
    skillSections.forEach((section) => {
      // FIXED: Process skills content properly
      ...
      // Check for commas in original content AND minimum number of skills
      const shouldUseCommaFormat = section.content.some(line => line.includes(',')) && allSkills.length > 3;

      if (shouldUseCommaFormat) {
        // ✅ Use comma-separated format (what AI outputs)
        const skillsText = allSkills.join(', ');
        const wrappedSkills = doc.splitTextToSize(skillsText, sidebarWidth - 25);
        wrappedSkills.forEach((skillLine) => {
          if (sidebarY > pageHeight - 30) return;
          doc.text(skillLine, 20, sidebarY);
          sidebarY += 4.5;
        });
      } else {
